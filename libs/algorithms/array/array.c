...
void append_(int *a, size_t *const n, const int value) {
    a[*n] = value;
    (*n)++;
}
...
size_t binarySearch_(const int *a, size_t n, int x) {
    ...
    return right != n && a[right] == x ? right : SIZE_MAX;
}
...
int* copyArray_(const int *originalArray, const size_t n) {
    int *copy = (int*) malloc(sizeof(int) * n);
    memcpy(copy, originalArray, sizeof(int) * n);
